...
(see transactions.replace_avanza_range), so it is idempotent. Insättning rows are
summed as total deposits (reported, not stored).
"""
...
def parse_avanza_csv(content: bytes | str) -> dict[str, Any]:
    """Parse into transaction rows. Returns {transactions, deposits_total,
    date_min, date_max, rows}. Pure (no DB) — safe for preview."""
